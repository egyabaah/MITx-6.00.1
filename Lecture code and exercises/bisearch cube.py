...
for n in range(len(option)):
    if option[n:] == "yes" or option[n:n + 2] == "ye" or option[n] == "y" or option[n:] == "Yes" or option[
                                                                                                    n:n + 2] == "Ye" or \
            option[n] == "Y" or option[n:] == "YES" or option[n:n + 2] == "YE":
        user_epilson = float(input("Type your increment value which should be an integer less than 0: "))
        epilson = user_epilson
if x < 1:
    high = 1
    low = x
    ans = (high + low) / 2
    while abs(ans ** 3 - x) >= epilson:
